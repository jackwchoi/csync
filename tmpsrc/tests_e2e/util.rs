...
pub fn check_report_line(line: &str, value: f64, unit: &str) {
    ...
    assert!(line.ends_with(&adj_unit), "line does not end with {}", adj_unit);
    assert!(line.contains(&adjusted_value), "line does not contain {}", adjusted_value);
}
        ...
        match output.status.code().unwrap() {
            ...
            0 => {
                // check the number of files synced
                {
                    ...
                    dbg!(&cipher_file_count);
                    dbg!(&file_count_line);
                    dbg!(&source_file_count);
